# Stereo schematic completion report

The working project now contains the stereo Rev 1 schematic: `DiscreteClassD.kicad_sch`, `PowerOutput_L.kicad_sch`, and `PowerOutput_R.kicad_sch`. The original mono `PowerOutput.kicad_sch` and PCB remain unchanged. The final three-page PDF is `output/pdf/DiscreteClassD-stereo.pdf`.

## Confirmed engineering choices

- Both OPA1656 packages use **+12V_GD**, with pin 8 on that rail and pin 4 on PGND. Their local 100 nF and 4.7 µF bypass capacitors follow the 12 V rail. VREF remains nominal 2.50035 V; comparators and logic use +5V_A. This resolves the original 5 V input common-mode limit, as authorized by the user.
- All eight MOSFETs are **DMN6070SY**. Manufacturer datasheet DS39440 Rev. 3-2 page 1 was downloaded and visually checked against the actual KiCad SOT-89-3 pad geometry: **1 = gate, 2 = drain including the tab, 3 = source**. No physical pin-number changes were needed. Datasheet/verification fields were added to the MOSFET symbols.
- AGENTS.md now records DMN6070SY, the stereo four-half-bridge arrangement, and the approved OPA1656 supply.
- This completes the requested schematic implementation. Output power, dead time, thermal behavior, and startup still require hardware validation; ERC is not a hardware sign-off.

## Verification performed

- Original baseline ERC: 72 violations (65 errors, 7 warnings).
- Final KiCad 10.0.5 ERC: **0 errors, 0 warnings, no exclusions added**. See `erc.json` for the full three-sheet report. Existing project settings were preserved: `single_global_label`, `four_way_junction`, `simulation_model_issue`, and `footprint_filter` were already ignored. No checks were disabled by this task.
...
|---|---|---|---|
| U1 OPA1656 | LEFT follower: 3 IN+, 2 IN−, 1 OUT | RIGHT follower: 5 IN+, 6 IN−, 7 OUT | Pin 8 +12V_GD; pin 4 PGND |
| U3 OPA1656 | VREF buffer: 3 IN+, 2 IN−, 1 OUT | Integrator: 5 IN+, 6 IN−, 7 OUT | Pin 8 +12V_GD; pin 4 PGND |
...
Vout = 1.185 × (1 + Rtop/Rbottom). Divider current is 118.5 µA, comfortably above the required 5 µA minimum. Actual output includes regulator accuracy, resistor tolerance, and feedback bias current. 
...
Two shared Panasonic FR EEUFR1V102, 1000 µF/35 V capacitors are at main power entry. These are radial THT bulk capacitors, preserving the existing selected part; ordinary new R/C passives use metric SMD footprints. The original C17 and J14 are moved into the main sheet, not duplicated per output channel.

Each TPS7A4901 is limited to 150 mA and dissipates approximately (24 − Vout) × Iout. At 100 mA, the 12 V regulator dissipates about 1.20 W; at 40 mA, the 5 V regulator dissipates about 0.76 W. These are examples, not verified operating currents. Gate-driver rail budgeting must include 8 × Qg × f, four driver operating currents, and the four OPA1656 amplifier-section currents. DMN6070SY nominal Qg = 12.3 nC at 10 V gives about 39.7 mA for eight gates at 403.7 kHz. Adding approximately 15.6 mA for the four OPA1656 sections and approximately 10.8 mA for the four drivers (using the datasheet typical VDD plus bootstrap no-load operating currents at 500 kHz) gives a preliminary 66.1 mA before other dynamic loads, bias currents, and the extra gate charge at 12 V. At that indicative current the 12 V regulator dissipates about 0.79 W. This is a typical estimate, not a guaranteed worst-case budget; verify current and PCB thermal performance. Thermal/current validation remains outstanding.
...
The completed schematic has **193 physical components**, including **30 DNP**: 14 ICs, 8 MOSFETs, 53 resistors, 67 capacitors, 12 diodes, 2 coupled inductors, 4 connectors, and 33 test points. Power flags are excluded from these totals. Section units of a package are counted once. Exact inventory follows, including existing/reused parts and every new schematic component. All physical component additions are represented here, including DNP options. The PCB has not been changed.
...
| C20 | 1000u / 35V FR | Main | Added | Fit | Capacitor_THT:CP_Radial_D12.5mm_P5.00mm |
| C21 | 10u / 50V | Main | Added | Fit | Capacitor_SMD:C_1210_3225Metric |
| C22 | 10n C0G | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C23 | 10n C0G | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C24 | 10u / 25V | Main | Added | Fit | Capacitor_SMD:C_1210_3225Metric |
| C25 | 10u / 50V | Main | Added | Fit | Capacitor_SMD:C_1210_3225Metric |
| C26 | 10n C0G | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C27 | 10n C0G | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C28 | 10u / 25V | Main | Added | Fit | Capacitor_SMD:C_1210_3225Metric |
| C29 | 10u / 16V | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C30 | 100n / 16V | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C31 | 1n C0G / 1% | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C32 | 100n / 25V | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C33 | 4u7 / 25V | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C34 | 100n / 25V | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C35 | 4u7 / 25V | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C36 | 100n / 25V | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C37 | 100n / 25V | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C38 | 100n / 25V | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C39 | 100n / 25V | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C40 | 220n FILM | Main | Added | Fit | Capacitor_SMD:C_2220_5750Metric |
| C41 | 100p C0G | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C42 | 100p C0G | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C43 | 100p C0G | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C44 | 100n / 25V | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C45 | 220n FILM | Main | Added | Fit | Capacitor_SMD:C_2220_5750Metric |
| C46 | 100p C0G | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C47 | 100p C0G | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C48 | 100p C0G | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
| C49 | 100n / 25V | Main | Added | Fit | Capacitor_SMD:C_0805_2012Metric |
...
| D20 | 1N4148W | Main | Added | Fit | Diode_SMD:D_SOD-123 |
| D21 | 1N4148W | Main | Added | Fit | Diode_SMD:D_SOD-123 |
| D22 | 1N4148W | Main | Added | Fit | Diode_SMD:D_SOD-123 |
| D23 | 1N4148W | Main | Added | Fit | Diode_SMD:D_SOD-123 |
...
| R20 | 91k2 / 0.1% | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R21 | 10k / 0.1% | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R22 | 32k2 / 0.1% | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R23 | 10k / 0.1% | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R24 | 10k | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R25 | 10k | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R26 | 10k / 0.1% | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R27 | 49k9 / 0.1% | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R28 | 3k09 / 0.1% | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R29 | 47k | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R30 | 10k | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R31 | 100R | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R32 | 330R | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R33 | 330R | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R34 | 10k | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R35 | 1k DNP | Main | Added | DNP | Resistor_SMD:R_0805_2012Metric |
| R36 | 47k | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R37 | 10k | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R38 | 100R | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R39 | 330R | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R40 | 330R | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R41 | 10k | Main | Added | Fit | Resistor_SMD:R_0805_2012Metric |
| R42 | 1k DNP | Main | Added | DNP | Resistor_SMD:R_0805_2012Metric |
...
| TP20 | +5V_A | Main | Added | Fit | TestPoint:TestPoint_Pad_D1.5mm |
| TP21 | VREF_2V5 | Main | Added | Fit | TestPoint:TestPoint_Pad_D1.5mm |
| TP22 | OSC_SQUARE | Main | Added | Fit | TestPoint:TestPoint_Pad_D1.5mm |
| TP23 | TRIANGLE_400K | Main | Added | Fit | TestPoint:TestPoint_Pad_D1.5mm |
| TP24 | AUDIO_L | Main | Added | Fit | TestPoint:TestPoint_Pad_D1.5mm |
| TP25 | PWM_L | Main | Added | Fit | TestPoint:TestPoint_Pad_D1.5mm |
| TP26 | L_DRIVE_P | Main | Added | Fit | TestPoint:TestPoint_Pad_D1.5mm |
| TP27 | L_DRIVE_N | Main | Added | Fit | TestPoint:TestPoint_Pad_D1.5mm |
| TP28 | ENABLE_L | Main | Added | Fit | TestPoint:TestPoint_Pad_D1.5mm |
| TP29 | AUDIO_R | Main | Added | Fit | TestPoint:TestPoint_Pad_D1.5mm |
| TP30 | PWM_R | Main | Added | Fit | TestPoint:TestPoint_Pad_D1.5mm |
| TP31 | R_DRIVE_P | Main | Added | Fit | TestPoint:TestPoint_Pad_D1.5mm |
| TP32 | R_DRIVE_N | Main | Added | Fit | TestPoint:TestPoint_Pad_D1.5mm |
| TP33 | ENABLE_R | Main | Added | Fit | TestPoint:TestPoint_Pad_D1.5mm |
...
| U7 | TLV3602DGKR | Main | Added | Fit | Package_SO:VSSOP-8_3x3mm_P0.65mm |
| U8 | TLV3602DGKR | Main | Added | Fit | Package_SO:VSSOP-8_3x3mm_P0.65mm |
| U9 | SN74LVC1G04DBVR | Main | Added | Fit | Package_TO_SOT_SMD:SOT-23-5 |
| U10 | SN74LVC2G17DBVR | Main | Added | Fit | Package_TO_SOT_SMD:SOT-23-6 |
| U11 | TPS7A4901DGNR | Main | Added | Fit | Package_SO:HVSSOP-8-1EP_3x3mm_P0.65mm_EP1.57x1.89mm |
| U12 | TPS7A4901DGNR | Main | Added | Fit | Package_SO:HVSSOP-8-1EP_3x3mm_P0.65mm_EP1.57x1.89mm |
...
- [Diodes DMN6070SY datasheet](https://www.diodes.com/datasheet/download/DMN6070SY.pdf): saved MOSFET characteristics. Top-view gate/drain/source arrangement verified against the assigned SOT-89-3 footprint.
- Supplied `sources/ucc27301a.pdf` and `sources/CSAD0660.pdf`, plus the newly saved `sources/DMN6070SY.pdf`, support the output-stage parts. The older DMTH6016LPS datasheet is historical and does not describe the MOSFETs used in this schematic.

No TPS7A4901 physical pin-number uncertainty was found. The user's alternative DRB custom symbol contains additional exposed-pad contact numbers; it was not used or assumed interchangeable with DGN. The DGN copy uses the verified mapping above. Both engineering decisions are resolved and implemented. No remaining physical IC or MOSFET pin-map uncertainty was identified for the symbols in use. Exact film/filter capacitor parts, effective ceramic capacitance, thermal design, output power, oscillator startup, bootstrap behavior, and final dead time remain hardware/component-release checks.

## Delivered files and preservation

- Working schematic: `DiscreteClassD.kicad_sch` with separate `PowerOutput_L.kicad_sch` and `PowerOutput_R.kicad_sch`.
- PDF: `output/pdf/DiscreteClassD-stereo.pdf`.
- Report and evidence: this folder contains `erc.json`, `netlist.xml`, `connectivity-check.json`, and `intended-main.json`.
- The schematic baseline and original AGENTS.md are preserved in `output/verification/pre-stereo-20260908`.
- `PowerOutput.kicad_sch`, `DiscreteClassD.kicad_pcb`, `DiscreteClassD.kicad_pro`, and `DiscreteClassD.kicad_prl` retain their original SHA-256 values. No footprint library geometry, board placement, routing, zones, feedback, protection circuitry, or volume control was changed.
- The earlier `stereo-draft` folder is a historical review artifact. Use the working schematic and final PDF above.

Additional bench note: the DMN6070SY datasheet lists typical turn-off delay and fall time of 35 ns and 11 ns under its stated 20 ohm test conditions. These are not the actual delays of this 10 ohm gate circuit, but reinforce that 20–30 ns logic dead time is provisional. Measure gate-source waveforms and switching overlap in both channels before choosing final RC values.
